	...
	var args = {email: req.session.username};
		  soap.createClient(baseURL,option, function(err, client) {
		      if(err){
		    	  ...
		    	  client.getUserOrders(args, function(err, result) {
		    		  console.log("Inside if soap method ");
			    	  if(err){
			    		  console.log("Error from soap method" + err);
			    	  }else{
			    		  console.log("response "+JSON.parse(result.getUserOrdersReturn));
			    		  json_response = {'statusCode': '200', 'product': JSON.parse(result.getUserOrdersReturn)};
